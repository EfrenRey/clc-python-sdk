...
class PublicIPs(object):
	...
	def __init__(self, server, public_ips_lst, token=None):
		self.token = token
		self.server = server
		self.public_ips = []
		...
		for public_ip in public_ips_lst:
			if 'public' in public_ip:
				self.public_ips.append(PublicIP(
					id=public_ip['public'],
					parent=self,
					public_ip_obj=public_ip,
					token=self.token)
				)
	...
	def Add(self, ports, source_restrictions=None, private_ip=None):
		...
		* public_ips.public_ips will not be updated to reflect this addition. Recreate object after request completes
		...
		for port in ports:
			if 'port_to' in port:
				payload['ports'].append({'protocol': port['protocol'], 'port': port['port'], 'portTo': port['port_to']})
			else:
				payload['ports'].append({'protocol': port['protocol'], 'port': port['port']})

		if source_restrictions:
			payload['sourceRestrictions'] = source_restrictions

		if private_ip:
			payload['internalIPAddress'] = private_ip

		return clc.v2.Requests(
			clc.v2.API.Call(
				'POST',
				'servers/%s/%s/publicIPAddresses' % (self.server.alias,self.server.id),
				json.dumps(payload),
				token=self.token),
...
class PublicIP(object):
	...
	def __init__(self, id, parent, public_ip_obj=None, token=None):
	...
	def _Load(self, cached=True):
		...
		if not self.data or not cached:  
			self.data = clc.v2.API.Call(
				'GET',
				'servers/%s/%s/publicIPAddresses/%s' % (self.parent.server.alias, self.parent.server.id, self.id),
				token=self.token)
			...
			for port in self.data['_ports']:  
				if 'portTo' in port:
					self.ports.append(Port(self, port['protocol'], port['port'], port['portTo'], token=self.token))
				else:
					self.ports.append(Port(self, port['protocol'], port['port']))
	...
	def Update(self):
		...
		return clc.v2.Requests(
			clc.v2.API.Call(
				...
				'servers/%s/%s/publicIPAddresses/%s' % (self.parent.server.alias, self.parent.server.id, self.id),
				json.dumps({
					'ports': [o.ToDict() for o in self.ports],
	...
	def AddPort(self, protocol, port, port_to=None):  
		...
		self.ports.append(Port(self, protocol, port, port_to, token=self.token))
	...
	def AddPorts(self,ports):  
		...
		for port in ports:  
			if 'port_to' in port:
				self.ports.append(Port(self, port['protocol'], port['port'], port['port_to'], token=self.token))
			else:
				self.ports.append(Port(self, port['protocol'], port['port'], token=self.token))
...
class Port(object):
	...
	def __str__(self):
		if self.port_to:
			return "%s-%s/%s" % (self.port, self.port_to, self.protocol)
		else:
